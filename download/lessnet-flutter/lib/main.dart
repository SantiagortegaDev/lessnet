...
class _ChatPageState extends State<ChatPage> {
  ...
  Widget _buildVideoContent(ChatMessage m) {
    return GestureDetector(
      onTap: () => _openVideoPlayer(m.filePath, m.fileName ?? 'Video'),
      child: Container(
        width: 240,
        height: 160,
        decoration: BoxDecoration(
          ...
          children: [
            ...
            if (m.filePath != null)
              FutureBuilder<VideoPlayerController?>(
                future: _getVideoThumbnail(m.filePath!),
                builder: (_, snap) {
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.videocam, color: Colors.white54, size: 12),
  ...
  /// Creates a temporary video controller just for thumbnail display.
  /// The controller is disposed after the widget tree reads the first frame.
  Future<VideoPlayerController?> _getVideoThumbnail(String path) async {
    try {
      final ctrl = VideoPlayerController.file(File(path));
      await ctrl.initialize();
      await ctrl.seekTo(const Duration(milliseconds: 500));
      ctrl.pause();
      return ctrl;
    } catch (_) {
      return null;
  ...
  /// Opens a full-screen video player page
  void _openVideoPlayer(String? path, String fileName) {
    if (path == null) return;
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => _VideoPlayerPage(filePath: path, fileName: fileName),
    ));
  }
